...
: ${KEYSTONE_DB_USER:=$KEYSTONE_USER}
: ${DB_ROOT_PASSWORD:=password}
: ${MARIADB_SERVICE_HOST:=$PUBLIC_IP}
: ${KEYSTONE_DB_PASSWORD:=$DB_ROOT_PASSWORD}
...
echo "Setting up PKI"
/usr/bin/keystone-manage pki_setup --keystone-user ${KEYSTONE_USER} --keystone-group keystone
...
crux user-create --update \
    -n ${ADMIN_TENANT_NAME} -p "${KEYSTONE_ADMIN_PASSWORD}" \
